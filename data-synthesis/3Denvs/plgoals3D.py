...
def generate_worlds(obstacles, n, n_goals, trials=30):
    ...
    for _ in range(trials):
        ...
        while(True):
            ...
            agent_z = random.randint(0, n-1)
            if([agent_x, agent_y, agent_z] not in obstacles):
                break

        # So we mark our obstacles as 1, our agent as 2, and open spaces as 0
        grid[agent_x][agent_y][agent_z] = 2
        ...
        while(True):
            ...
            goal_z = random.randint(0, n-1)
            if([goal_x, goal_y, goal_z] not in obstacles 
                    and [goal_x, goal_y, goal_z] != [agent_x, agent_y, agent_z]
                    and [goal_x, goal_y, goal_z] not in goals):
                goals.append([goal_x, goal_y, goal_z])
                grid[goal_x][goal_y][goal_z] = 3
        ...
        worlds.append({
            'world': grid,
            'obstacles': obstacles,
            'start': [agent_x, agent_y, agent_z], # Just one
            'goals': goals
        })
