...
# Takes in an x by y by z world, an initial location as a list [x coord, y coord, z coord], a list of goals [goal index][x coord][y coord][z coord].
# They seem to ignore contraints here.
# Just simple string concatenation to generate. Straightforward.
def generate_nl(x, y, z, obstacles, goals, initial_loc, constraint='None'):
    ...
    english = f'You are in a {x} by {y} by {z} world. There are obstacles that you have to avoid at: '

    # First provide the obstacles.
    for i in range(len(obstacles)):
        obstacle = obstacles[i]
        english += f'({obstacle[0]},{obstacle[1]},{obstacle[2]})'
    ...
    english += '. '
    if(len(goals) == 1):
        english += f'Go from ({initial_loc[0]},{initial_loc[1]},{initial_loc[2]}) to ({goals[0][0]},{goals[0][1]},{goals[0][2]})'

    # If multiple goals
    else:
        english += f'You are at ({initial_loc[0]},{initial_loc[1]},{initial_loc[2]}).'
    ...
    # this is where the multiple goals are
        for i in range(len(goals)):
            english += f'p{i} is located at ({goals[i][0]}, {goals[i][1]}, {goals[i][2]})'
            if(i < len(goals) - 2):
                english += ', '
            elif (i == len(goals) - 2):
                english += ' and '
...
def a_star(grid, start, goal):
    ...
    while heap:
        ...
        for action in actions:
            neighbor = (current[0] + action[0], current[1] + action[1], current[2] + action[2])

            if (
                0 <= neighbor[0] < grid.shape[0]
                and 0 <= neighbor[1] < grid.shape[1]
                and 0 <= neighbor[2] < grid.shape[2]
            ):
                if grid[neighbor] != 1:
                    new_cost = cost + 1
...
def solution_direction(path):
    ...
    for i in range(len(path)):
        ...
        if(path[i] == 'right'):
            ...
            if(orientation == 'south'):
                directions += 'turn down move forward '
            elif (orientation == 'north'):
                directions += 'turn up move forward '
            elif (orientation == 'east'):
                directions += 'turn right move forward '
            elif (orientation == 'west'):
                directions += 'turn left move forward '
...
def main():
    ...
    with open(str(sys.argv[1])) as f:
        ...
        for world in data:
            ...
            nl = generate_nl(len(grid), len(grid[0]), len(grid[0][0]), obstacles, goals, start)

            if(len(goals) == 1):
                # This puts in our grid, starting position, and goal position
                coordinates = a_star(np.array(grid), (start[0], start[1], start[2]), (goals[0][0], goals[0][1], goals[0][2]))
