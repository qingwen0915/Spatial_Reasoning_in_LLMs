...
eng = []
acts = []
eng = json.load(open('../data-synthesis/3Denvs/20_train_set_samples.json'))

with open('../data-synthesis/3Denvs/Nvidea_LLaMA3-few-shot-prompts.txt', 'r') as f:
    prompt = f.read()
...
"""
        ...
        print(word_count)
        with torch.no_grad():
            output = model.module.generate(input_ids, max_length = 420, num_return_sequences=1, eos_token_id=tokenizer.eos_token_id, stopping_criteria=stopping_criteria)
        #print("Got output")
...
with open('results_instruction_tuned_Nvidea_Fixed.json', 'w') as fo:
